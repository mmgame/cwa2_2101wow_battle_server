...
public class TrapEnergyHandler extends ATrapHandler {
	@Override
	public TrapTriggerBean handler(IPerformer performer, Trap trap) {
		...
		if (trapPrototype.getValueType() == ValueTypeEnum.Value_Num.value()) {
			...
			// 百分比
			changeValue = performer.getAttrMgr().getAttrFinalValue(AttrTypeEnum.Attr_Energy.value()) * trapPrototype.getDamageAmountList().get(0) / SimuiationConstant.Percent;
		}

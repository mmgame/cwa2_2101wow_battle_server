 ...
 *
 */
public class SevenResistanceBuff implements ISBuff {

	@Override
	public int getType() {
		return BuffTypeEnum.Buff_SevenResistance.value();
	}
	...
	public void enter(IBuffContext context) {
		...
		for(int i=0;i<list.size();i++){
			if(buffPrototype.getValueType() == ValueTypeEnum.Value_Num.value()){//数值
			...
			if(buffPrototype.getBuffOrDebuff() == BuffBeneficialTypeEnum.Beneficial_Bad.value()){//减益
				changeValue = -changeValue;
			}
			performer.getAttrMgr().setElementResistanceValue(AttrSourceTypeEnum.AS_Buff.value(),list.get(i), changeValue);
		}
	}
	...
	public void exit(IBuffContext context) {
		...
		for(int i=0;i<list.size();i++){
			...
			if(buffPrototype.getBuffOrDebuff() == BuffBeneficialTypeEnum.Beneficial_Good.value()){//增益
				changeValue = -changeValue;
			}
			performer.getAttrMgr().setElementResistanceValue(AttrSourceTypeEnum.AS_Buff.value(),list.get(i), changeValue);
		}

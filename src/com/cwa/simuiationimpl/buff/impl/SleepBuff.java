 ...
 *
 */
public class SleepBuff implements ISBuff {

	@Override
	public int getType() {
		return BuffTypeEnum.Buff_Sleep.value();
	}

	@Override
	public void enter(IBuffContext context) {
		IPerformer performer = context.getPerformer();
		performer.getAttrMgr().setAttrValue(AttrSourceTypeEnum.AS_Buff.value(), AttrTypeEnum.Attr_HitRate.value(), BuffConstant.SleepHurtRate);
		performer.getAttrMgr().changeConsumeValue(AttrTypeEnum.Attr_HitRate.value(), BuffConstant.SleepHurtRate);
	...
	public void exit(IBuffContext context) {
		context.getPerformer().getAttrMgr().setAttrValue(AttrSourceTypeEnum.AS_Buff.value(), AttrTypeEnum.Attr_HitRate.value(), -BuffConstant.SleepHurtRate);
		context.getPerformer().getAttrMgr().changeConsumeValue(AttrTypeEnum.Attr_HitRate.value(), -BuffConstant.SleepHurtRate);
	}

 ...
 * 
 */
public class ESEffectPurgeHandler extends AEffectHandler {

	@Override
	public int getEffectType() {
		return EffectTypeEnum.Type_Purge.value();
	}
	...
	public void handler(IPerformer source, AffectedBean affectedBean,ESMagicEvent event) {
		...
		for (IPerformer target : magicEffectBean.getPerformers()) {
			...
			List<Integer> buffs = target.getBuffManager().dispelBuff(BuffBeneficialTypeEnum.Beneficial_Bad, null);
			if(changeValue > 0){
				...
				targetPEffectBean.addEffects(HurtTypeEnum.Effect_Recover.value());
